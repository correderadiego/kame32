...
void ServoHandler::moveServos(float servo_position[NUM_SERVOS]) {    
    for (int i = 0; i < NUM_SERVOS; i++){
        servo[i]->move(servo_position[i]);
    } 
}

void ServoHandler::moveServosDelayed(int ms, float target[NUM_SERVOS]) {    
    if (ms>10){
        for (int i=0; i<NUM_SERVOS; i++)
